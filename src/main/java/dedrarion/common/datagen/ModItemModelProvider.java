...
public class ModItemModelProvider extends net.minecraftforge.client.model.generators.ItemModelProvider {
    public ModItemModelProvider(PackOutput output, ExistingFileHelper existingFileHelper) {
        super(output, Dedrarion.MOD_ID, existingFileHelper);
    }
    ...
    private void simpleItem(RegistryObject<Item> item){
        ...
        withExistingParent(item.getId().getPath(),
                ...
                ResourceLocation.fromNamespaceAndPath(Dedrarion.MOD_ID, "item/" + item.getId().getPath()));
    }

    public void evenSimplerBlockItem(RegistryObject<StairBlock> block) {
        this.withExistingParent(Dedrarion.MOD_ID + ":" + Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block.get())).getPath(),
